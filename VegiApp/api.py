...
def checkAuthorization(request_header):
    ...
    if(checkAuthKey(request_header) == True):
        if(request_header['Authorization'] != "" and len(request_header['Authorization']) == 128):
            try:
                token = AuthTokens.objects.get(tokenAuth = request_header['Authorization'])
                access = True
...
class APILogin(APIView):
    def get(self, request):
        response = {'error': 'Access Denied.! Request not granted for GET method.'}
        return Response(response)
    
    def put(self, request):
        response = {'error': 'Access Denied.! Request not granted for PUT method.'}
        return Response(response)
    
    def delete(self, request):
        response = {'error': 'Access Denied.! Request not granted for DELETE method.'}
        return Response(response)

    def post(self, request):
        model = User
        response = ""
        userCount, userId = checkUser(model, request.data['userEmail'], request.data['userPassword'])
